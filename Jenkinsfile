...
pipeline {
    ...
    environment {
        ...
        IMAGE_NAME = "sreyassharma/signed_images_jenkins:1.0.1"
        ...
        stage("Grype Scan") {
            steps {
                script {
                    ...
                    bat "grype ${IMAGE_NAME} -o json > reports\\grype_report.json || exit 0"
                    bat "grype ${IMAGE_NAME} -o table > reports\\grype_report.txt || exit 0"

                    def foundHigh = bat(returnStatus: true,
